...
Šum je generátor náhodných čísel počítačové grafiky. Jedná se o náhodný a neuspořádaný vzor, který je užitečný všude tam, kde je potřebný detail bez evidentní struktury.
\subsection{Perlinův šum}
\label{perlinNoise}
Roku 1985 Perlin představil \textit{Perlinův šum}, jeho slavnou procedurální funkci pro generování šumu.~\cite{PerlinKen}~\cite{Perlin2002ImprovingN}

\subsection{Porovnání procedurálních funkcí šumů}
Tato sekce se věnuje porovnání jednotlivý funkcí procedurálního generování šumů.~\cite{Lagae10}

Šumy popsané v jednotlivých citovaných pracích a následně výsledky jejich porovnání zapisovány do tabulky 

\begin{table}
	\centering
	\caption{Porovnání funkce šumu}
	\label{tab:porovnani_sumu}
	\begin{tabular}{|m{5em}|m{5em}|m{5em}|m{5em}|m{5em}|m{5em}|m{5em}|}
		\hline
		\textbf{Kategorie} & \textbf{Perlinův šum} \cite{Perlin2002ImprovingN} & \textbf{Řídký konvoluční šum} \cite{Lewis89} & \textbf{Vlnkový (Wavelet) šum} \cite{Cook05} & \textbf{Anisotropní šum} \cite{Goldberg08} & \textbf{Kvalitnější gradientový šum} \cite{Kensler2008} & \textbf{Gaborův šum} \cite{Lagae09} \\
		\hline
		\textbf{Požadavky na úložný prostor} & & & & & & \\
		Procedurální & $O(N)$ & $O(1)$ & $O(Nd)$ & $O(Nd)$ & $O(dN)$ & $O(1)$ \\
		Parametry & Váhový součet & Kernel & Váhový součet & Váhový součet (aniso.) & Váhový součet & Kernel \\
		\hline
		\textbf{Modelování} & & & & & & \\
		Spektrální kontrola & Váhový součet & Kernel & Váhový součet & Váhový součet (aniso.) & Váhový součet & Kernel \\
		Anizotropní šum & & $\checkmark$ & & $\checkmark$ & & \\
		\hline
		\textbf{Povrchy} & & & & & & \\
		Pevný šum & $\checkmark$ & $\checkmark$ & $\checkmark$ & & $\checkmark$ & $\checkmark$ \\
		Povrchový šum & & $\checkmark$ & $\checkmark$ & & $\checkmark$ & $\checkmark$ \\
		Bez nastavení povrchového šumu & & & $\checkmark$ & & $\checkmark$ & $\checkmark$ \\
		\hline
		\textbf{Filtrování} & & & & & & \\
		Izotropní filtrování & & $\checkmark$ & $\checkmark$ & $\checkmark$ & & \\
		Anizotropní filtrování & & & & $\checkmark$ & & $\checkmark$ \\
		\hline
		\textbf{Analýza} & & & & & & \\
		Spektrum pásmové propusti & & $\checkmark$ & $\checkmark$ & $\checkmark$ & $\checkmark$ & \\
		Gaussovské rozložení amplitudy & & $\checkmark$ & $\checkmark$ & $\checkmark$ & $\checkmark$ & $\checkmark$ \\
		\hline
		\textbf{Porovnání} & & & & & & \\
		Rychlost & 0 & $-$ & $-$ & $+$ & $-$ & $-$ \\
		Obchod kvalita/rychlost & $\checkmark$ & & & & & $\checkmark$ \\
		\hline
	\end{tabular}
\end{table}
